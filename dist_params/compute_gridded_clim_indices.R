...
# compute climate indices for each grid cell - takes ~10min
indices<-dt[!is.na(tas),compute_climate_indices_berghuijs(tas,pr,pet,time,0.05),by=.(latitude,longitude)]
...
#save(file=paste0('/gpfs/ts0/projects/Research_Project-CLES-00008/conus/indices/clim_indices_maurer_',start_year,'-',end_year,'_indices_only.Rdata'),indices)
...
#Really unprojected square plot-  https://stackoverflow.com/questions/43612903/how-to-properly-plot-projected-gridded-data-in-ggplot2
p_aridity<-ggplot(indices, aes(y=latitude, x=longitude, fill=aridity)) +
  geom_tile() + theme_bw() + scale_fill_fermenter(palette='BrBG',breaks=c(0.5,0.75,1,1.5,2)) +
  geom_polygon(data = usa, aes(x=long, y = lat, group = group), fill = NA, color = "black")  +
  coord_fixed(1.3)

p_snow<-ggplot(indices, aes(y=latitude, x=longitude, fill=frac_snow_daily)) +
  geom_tile() + theme_bw() + scale_fill_distiller(palette='Blues',direction=1) +
  geom_polygon(data = usa, aes(x=long, y = lat, group = group), fill = NA, color = "black")   +
  coord_fixed(1.3)

p_season<-ggplot(indices, aes(y=latitude, x=longitude, fill=p_seasonality)) +
...
ggsave(paste0(dir_plots,'fuse_gmd/indices.pdf'),grid.arrange(p_season, p_aridity, p_snow, nrow = 2))
